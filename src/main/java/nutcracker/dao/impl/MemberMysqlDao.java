...
public class MemberMysqlDao implements MemberDao {
  ...
  public ArrayList<Member> getList() throws Exception {
    ...
    try (
      PreparedStatement stmt = con.prepareStatement(
          "select mno, email, name, lst_date" +
          " from memb"); 
      ...
      while (rs.next()) { // 서버에서 레코드 한 개를 가져왔다면,
        ...
        member.setLastDate(rs.getTimestamp("lst_date"));
  ...
  public boolean exist(int memberNo) throws Exception {
    ...
    try (
      PreparedStatement stmt = con.prepareStatement(
          "select count(*) from memb where mno=?"); ) {
  ...
  public void insert(Member member) throws Exception {
    ...
    try (
      PreparedStatement stmt = con.prepareStatement(
          "insert into memb(email,name,pwd,lst_date) values(?,?, password(?),?)",
          Statement.RETURN_GENERATED_KEYS); ) {
      
      stmt.setString(1, member.getEmail());
      stmt.setString(2, member.getName());
      stmt.setString(3, member.getPassword());
      stmt.setTimestamp(4, member.getLastDate());
      stmt.executeUpdate();
  ...
  public void update(Member member) throws Exception {
    ...
    try (
      PreparedStatement stmt = con.prepareStatement(
          "update memb set"
          + " email=?, name=?, pwd=password(?), lst_date=?"
          + " where mno=?"); ) {
  ...
  public Member getOne(String email, String password) throws Exception {
    ...
    try (
      PreparedStatement stmt = con.prepareStatement(
          "select mno, email, name, lst_date from memb where email=? and pwd=password(?)"); ) {
