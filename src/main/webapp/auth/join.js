...
// 패스워드 8자 유효성검사
function checkPassword() {                             // 함수를 선언한다.
  var elMsg = document.getElementById('feedback');     // feedback 요소를 가져온다.
  var elPassword = document.getElementById('password');// username 입력 창을 가져온다.
  if (elPassword.value.length < 8 || elPassword.value.length > 20) {                   // 이름의 길이가 5글자 미만이면
    elMsg.textContent = '비밀번호는 8글자 이상 20자 이하여야 합니다.'; // 메시지를 표시한다.
  } else {                                              // 그렇지 않으면
...
function setup() {
  var textInput;
  textInput = document.querySelectorAll('.required');
}
